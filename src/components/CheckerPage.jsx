...
function CheckpointRow({ item, existing, submitted, onSaved }) {
  ...
  const [currentValue, setCurrentValue] = useState(existing?.current_value || "");
        ...
        <div style={{ flex: 1, minWidth: 200 }}>
          ...
          {(item.sop_ref || item.limit_value || item.instrument) && (
            <div style={{ fontSize: 11, color: "#94a3b8", marginTop: 2 }}>
              {[item.sop_ref, item.limit_value && `Limit: ${item.limit_value}`, item.instrument && `Instrument: ${item.instrument}`]
                .filter(Boolean)
                .join(" · ")}
            </div>
          )}
        </div>
        <ResultButtons value={result} onChange={save} disabled={submitted || saving} />
      </div>
      ...
      {isAbnormal && (
        <div style={{ display: "flex", gap: 8, marginTop: 10, flexWrap: "wrap" }}>
          <input
            ...
            disabled={submitted}
            style={{ flex: 1, minWidth: 120, padding: "6px 10px", borderRadius: 7, border: "1.5px solid #fecaca", fontSize: 12, fontFamily: F, outline: "none" }}
          />
          <input
            value={remarks}
            ...
            placeholder="Remarks…"
            disabled={submitted}
            style={{ flex: 2, minWidth: 160, padding: "6px 10px", borderRadius: 7, border: "1.5px solid #fecaca", fontSize: 12, fontFamily: F, outline: "none" }}
          />
        </div>
      )}

      {saving && <div style={{ fontSize: 10, color: "#d97706", marginTop: 6 }}>Saving…</div>}
...
function TopNav({ checkedBy, onNavigate }) {
  ...
  return (
    <div style={{ background: "#0B1F3A", height: 56, display: "flex", alignItems: "center", padding: "0 20px", gap: 12, flexShrink: 0 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        <div style={{ width: 32, height: 32, borderRadius: 6, background: "linear-gradient(135deg,#3b82f6,#22c55e)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15 }}>🔍</div>
        <div>
          <div style={{ color: "#fff", fontWeight: 800, fontSize: 15, fontFamily: F }}>SmartPM</div>
          <div style={{ color: "rgba(255,255,255,.5)", fontSize: 10, textTransform: "uppercase", letterSpacing: ".5px", fontFamily: F }}>Checker Portal</div>
        </div>
      </div>
      <div style={{ flex: 1 }} />
      <div style={{ display: "flex", alignItems: "center", gap: 6, background: "rgba(255,255,255,.08)", border: "1px solid rgba(255,255,255,.12)", borderRadius: 100, padding: "5px 12px 5px 8px" }}>
        <div style={{ width: 8, height: 8, borderRadius: "50%", background: "#22c55e" }} />
        <span style={{ color: "#fff", fontSize: 13, fontWeight: 700, fontFamily: F }}>{checkedBy} · Checker</span>
      </div>
      <button
        onClick={() => onNavigate && onNavigate("home")}
        style={{ fontSize: 13, fontWeight: 600, color: "rgba(255,255,255,.75)", background: "transparent", border: "none", padding: "5px 10px", borderRadius: 6, cursor: "pointer", fontFamily: F }}
      >
        ← Home
      </button>
      <div style={{ color: "rgba(255,255,255,.5)", fontSize: 13, fontFamily: F }}>{clock}</div>
    </div>
  );
}
...
function SidebarLabel({ children }) {
  return (
    <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: ".6px", textTransform: "uppercase", color: "#94a3b8", padding: "14px 20px 6px", fontFamily: F }}>
      {children}
    </div>
  );
}
...
function CheckerSidebar({ activeTab, setActiveTab, badges, onNavigate, onQuickAction }) {
  return (
    <aside style={{ width: 240, background: "#fff", borderRight: "1px solid #e2e8f0", overflowY: "auto", flexShrink: 0 }}>
      <SidebarLabel>My Tasks</SidebarLabel>
      {MY_TASKS.map((t) => (
        <SidebarLink key={t.id} icon={t.icon} label={t.label} badge={badges[t.id]} active={activeTab === t.id} onClick={() => setActiveTab(t.id)} />
      ))}
      ...
      <SidebarLink icon="📋" label="Planner View" onClick={() => onNavigate && onNavigate("planner")} />
      <SidebarLink icon="🔧" label="Executor View" onClick={() => onNavigate && onNavigate("executor")} />
      <SidebarLink icon="📊" label="Mentor Dashboard" onClick={() => onNavigate && onNavigate("dashboard")} />

      <div style={{ height: 1, background: "#e2e8f0", margin: "10px 20px" }} />
      <SidebarLabel>Quick Actions</SidebarLabel>
      <div style={{ padding: "4px 12px 16px" }}>
        <QuickActionButton icon="⚠️" label="Log Abnormality" onClick={() => onQuickAction("log-abnormality")} />
        <QuickActionButton icon="✅" label="Pending Audits" onClick={() => onQuickAction("pending-audits")} />
        <QuickActionButton icon="🧾" label="View History" onClick={() => onQuickAction("view-history")} />
      </div>
    </aside>
  );
}
...
function ComingSoon({ title, subtitle }) {
  return (
    <div style={{ padding: "28px 28px" }}>
      <div style={{ fontSize: 22, fontWeight: 800, color: "#0f172a", marginBottom: 4, fontFamily: F }}>{title}</div>
      <div style={{ fontSize: 14, color: "#64748b", marginBottom: 20, fontFamily: F }}>{subtitle}</div>
      <div style={{ fontSize: 14, color: "#94a3b8", fontFamily: F }}>This module hasn't been built yet.</div>
    </div>
  );
}
...
export default function CheckerPage({ checkedBy = "Checker", onNavigate }) {
  ...
  useEffect(() => {
    ...
    Promise.all([fetchEquipment(), fetchPlants(), fetchLines(), fetchMachines()])
      .then(([eqJ, plantJ, lineJ, machineJ]) => {
        const eqRows = rowsOf(eqJ).filter((e) => String(e.is_active ?? "1") === "1");
  ...
  const effectiveSelectedEquipment =
    filteredEquipment.find((eq) => (eq.equipment_code || eq.code) === selectedEquipment)
      ? selectedEquipment
  ...
  const progress = useMemo(() => {
    ...
    const done = checklistItems.filter((i) => resultsMap[i.cdb_object_id]).length;
    return { done, total };
  ...
  const pendingForCurrentEquipment = isSubmitted ? 0 : Math.max(progress.total - progress.done, 0);
  const abnormalCount = Object.values(resultsMap).filter((r) => r.result === "Abnormal").length;
  const badges = {
  ...
  const TAB_META = {
    "pm-inspection": { label: "PM Inspection", subtitle: `${checkedBy} · Checker` },
    abnormalities: { label: "Abnormalities", subtitle: "Flagged out-of-limit readings" },
    "iiot-alerts": { label: "IIoT Alerts", subtitle: "Sensor-triggered notifications" },
    consolidated: { label: "Consolidated List", subtitle: "All tasks across equipment" },
    "audit-queue": { label: "Audit Queue", subtitle: "Inspections awaiting audit" },
    "my-calendar": { label: "My Calendar", subtitle: "Scheduled inspections" },
    "history-status": { label: "History & Status", subtitle: "Past submissions" },
  };

  return (
    <div style={{ fontFamily: F, background: "#f8fafc", minHeight: "100vh", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ display: "flex", flex: 1, minHeight: 0 }}>
        ...
        <div style={{ flex: 1, minWidth: 0, overflowY: "auto" }}>
            ...
            <div>
              ...
              <div style={{ background: "#fff", borderBottom: "1.5px solid #e2e8f0", padding: "16px 20px" }}>
                <div style={{ fontSize: 12, color: "#94a3b8", marginBottom: 6 }}>
                  Checker <span style={{ margin: "0 4px" }}>›</span> {TAB_META[activeTab].label}
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16, flexWrap: "wrap" }}>
                  <div>
                    ...
                    <div style={{ fontSize: 13, color: "#64748b", marginTop: 2 }}>
                      {checkedBy} · Checker
                      {selectedEquipmentRow && ` · ${selectedEquipmentRow.plant_code || ""}`}
                    </div>
                  </div>
                  {effectiveSelectedEquipment && checklistItems.length > 0 && !isSubmitted && (
                    <button
                  ...
                  {isSubmitted && (
                    <span style={{ fontSize: 13, fontWeight: 700, color: AC }}>✓ Inspection Submitted</span>
                  )}
                </div>
              </div>

              {/* ── Pending/Abnormal/IIoT summary strip ── */}
              <div style={{ background: "#fff", borderBottom: "1.5px solid #e2e8f0", padding: "12px 20px", display: "flex", gap: 10, flexWrap: "wrap" }}>
                {[
                  { label: "Pending", value: pendingForCurrentEquipment, color: "#d97706" },
                  { label: "Abnormalities", value: abnormalCount, color: "#dc2626" },
                  { label: "IIoT Alerts", value: summary.iiot_alerts, color: "#2563eb" },
                ].map(({ label, value, color }) => (
                  <div key={label} style={{ background: `${color}12`, border: `1.5px solid ${color}33`, borderRadius: 10, padding: "6px 14px", textAlign: "center", minWidth: 80 }}>
              ...
              <div style={{ background: "#fff", borderBottom: "1px solid #e2e8f0", padding: "12px 20px", display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
                <span style={{ fontSize: 13, fontWeight: 700, color: "#64748b", textTransform: "uppercase", letterSpacing: 0.5 }}>Filter:</span>
                <select
                  value={filterPlant}
                  onChange={handlePlantFilterChange}
                  style={{ width: 160, fontSize: 13, padding: "5px 8px", border: "1.5px solid #cbd5e1", borderRadius: 8, color: "#334155", background: "#fff", outline: "none" }}
                >
                ...
                <select
                  ...
                  style={{ width: 150, fontSize: 13, padding: "5px 8px", border: "1.5px solid #cbd5e1", borderRadius: 8, color: "#334155", background: "#fff", outline: "none" }}
                >
                ...
                <span style={{ fontSize: 13, fontWeight: 700, color: "#64748b", textTransform: "uppercase", letterSpacing: 0.5 }}>Equipment:</span>
                <select
                  value={effectiveSelectedEquipment}
                  onChange={(e) => setSelectedEquipment(e.target.value)}
                  disabled={loading || filteredEquipment.length === 0}
                  style={{ width: 260, fontSize: 13, fontWeight: 700, padding: "5px 8px", border: "1.5px solid #cbd5e1", borderRadius: 8, color: "#0f172a", background: "#fff", outline: "none" }}
                >
                  {loading && <option value="">Loading equipment…</option>}
                  {!loading && filteredEquipment.length === 0 && <option value="">No equipment in this scope</option>}
                ...
                {(filterPlant !== "all" || filterLine !== "all" || filterMachine !== "all") && (
                  <button
                    onClick={() => { setFilterPlant("all"); setFilterLine("all"); setFilterMachine("all"); }}
                    style={{ fontSize: 13, fontWeight: 700, color: "#2563eb", background: "transparent", border: "none", cursor: "pointer", padding: "5px 0" }}
                  >
                    ✕ Clear
                  </button>
                )}
              ...
              {error && (
                <div style={{ background: "#fef2f2", borderBottom: "1px solid #fecaca", padding: "8px 20px", fontSize: 12, color: "#dc2626", fontWeight: 600 }}>
                  ⚠ {error}
                </div>
              )}
              ...
              {effectiveSelectedEquipment && (
                <div style={{ padding: "14px 20px 0" }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
                    <span style={{ fontSize: 12, fontWeight: 700, color: "#475569" }}>Progress</span>
                    <span style={{ fontSize: 12, fontWeight: 800, color: AC }}>{progress.done}/{progress.total}</span>
                  </div>
                  <div style={{ height: 8, background: "#e2e8f0", borderRadius: 4, overflow: "hidden" }}>
                    <div
                      style={{
                        width: `${progress.total > 0 ? (progress.done / progress.total) * 100 : 0}%`,
                        height: "100%", background: AC, borderRadius: 4, transition: "width .3s",
                      }}
                    />
                  </div>
                </div>
              ...
              <div style={{ padding: "16px 20px" }}>
                {loadingItems ? (
                  <div style={{ textAlign: "center", padding: "40px 0", color: "#94a3b8" }}>Loading checklist…</div>
                ) : !checklistHeaderFound ? (
                  <div style={{ textAlign: "center", padding: "40px 0", color: "#94a3b8" }}>
                    No checklist has been built for this equipment yet — create one in PM Checklist Builder.
                  </div>
                  ...
                  checklistItems.map((item) => (
                    <CheckpointRow
                      ...
                      item={{ ...item, equipment_code: effectiveSelectedEquipment, checked_by: checkedBy }}
          ...
          {activeTab === "abnormalities" && <ComingSoon title="Abnormalities" subtitle="Flagged out-of-limit readings across your equipment" />}
          {activeTab === "iiot-alerts" && <ComingSoon title="IIoT Alerts" subtitle="Sensor-triggered notifications requiring attention" />}
          {activeTab === "consolidated" && <ComingSoon title="Consolidated List" subtitle="All tasks across equipment, plants and frequencies" />}
          {activeTab === "audit-queue" && <ComingSoon title="Audit Queue" subtitle="Submitted inspections awaiting audit sign-off" />}
          {activeTab === "my-calendar" && <ComingSoon title="My Calendar" subtitle="Scheduled inspection dates by equipment" />}
          {activeTab === "history-status" && <ComingSoon title="History & Status" subtitle="Past submissions and their outcomes" />}
        </div>
      </div>
    </div>
  );
