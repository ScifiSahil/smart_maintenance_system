    ...
    <div className="grid [grid-template-columns:repeat(auto-fit,minmax(160px,1fr))] gap-3.5 mb-6">
      {cards.map((c) => (
        <div key={c.label} className="relative overflow-hidden bg-white border border-[#E2E8F0] rounded-xl py-[18px] px-5">
          ...
          <div className="text-sm font-semibold tracking-wide uppercase text-[#64748B] mb-1.5">{c.label}</div>
          <div className="text-3xl font-bold text-[#0B1F3A] leading-none" style={{ fontFamily: 'var(--font-mono)' }}>{c.value}</div>
          <div className="text-sm text-[#94A3B8] mt-1">{c.sub}</div>
        </div>
      ))}
    </div>
  );
